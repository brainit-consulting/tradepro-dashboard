...
function toast(msg, kind, opts) {
  ...
  function updateCheckpointCollapseUI() {
    const wrap = $("bCkptsWrap");
    const toggle = $("bCkptToggle");
    if (wrap) wrap.style.display = CKPTS_COLLAPSED ? "none" : "block";
    if (toggle) {
      toggle.textContent = CKPTS_COLLAPSED ? "Expand" : "Collapse";
      toggle.setAttribute("aria-expanded", (!CKPTS_COLLAPSED).toString());
    }
  }
...
function getBranchEditorCheckpointsFromDOM() {
  ...
  function addBranchEditorCheckpoint(text = "") {
    ...
    CKPTS_COLLAPSED = false;
    updateCheckpointCollapseUI();
  }
  ...
  function bind() {
    ...
    $("bCkptToggle")?.addEventListener("click", () => {
      CKPTS_COLLAPSED = !CKPTS_COLLAPSED;
      updateCheckpointCollapseUI();
    });
